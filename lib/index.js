...
var connectDo = cow(function*() {
    ...
    try {
        var data = {};
        if (!this.started) data = {startup: true, workers: Object.keys(this.workers)};
...
Server.prototype.sendMessage = cow(function*(path, data, override) {
    ...
    try {
        ...
        if (this.connected) ignoreErrors(this.connect(), Errors.Cancelled).done();
        throw err;
    }
});
...
Server.prototype._sendMessage = cow(function*(path, data) {
    ...
    this.log('Sending message', {path: path, data: data});
    ...
    // done
    this.log('Received response data', result.data);
    return result.data;
});
