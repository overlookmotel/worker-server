...
Job.prototype.start = cow(function*() {
	// Run job
	var logObj = Object.assign({}, this);
	delete logObj.server;
	this.log('Starting job', logObj);
...
Job.prototype.failed = cow(function*(err) {
	var status = (err instanceof Errors.Job.Cancelled) ? 'Cancelled' : 'Failed';
	yield this.finished(status, {error: _.extend({message: err.message, name: err.name, stack: err.stack}, err)});
});
